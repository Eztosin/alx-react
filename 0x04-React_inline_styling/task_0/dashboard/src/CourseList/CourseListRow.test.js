...
describe("Course List Row component test", () => {
  ...
  it("should have background color #f5f5f5ab when isHeader is false", () => {
    const wrapper = shallow(<CourseListRow isHeader={false} textFirstCell="test" textSecondCell="test" />);
    const rowStyle = wrapper.find("tr").prop("style");

    expect(rowStyle).toHaveProperty("backgroundColor", "#f5f5f5ab");
  });

  it("should have background color #deb5b545 when isHeader is true", () => {
    const wrapper = shallow(<CourseListRow isHeader={true} textFirstCell="test" textSecondCell="test" />);
    const rowStyle = wrapper.find("tr").prop("style");

    expect(rowStyle).toHaveProperty("backgroundColor", "#deb5b545");
  });
});
